...
namespace PrimeNG.TableFilter.Test
{
    public class PrimeNGTableFilterIEnumerableTest
    {
        ...
        [Fact]
        public void EnumFilterShouldReturnFiveElement()
        {
            var filter = GenerateFilterTableFromJson("{ filters: { Enum1: { value: [3,2], matchMode: \"in\" }  } , first: 0, globalFilter: null, multiSortMeta: undefined, rows: 10,sortOrder: -1 }");
            var dataSet = GenerateMockTestData();
            dataSet = dataSet.PrimengTableFilter(filter, out var totalRecord);
            Assert.Equal(5, totalRecord);
        }

        [Fact]
        public void EnumFilterShouldReturnFiveElementV2()
        {
            var filter = GenerateFilterTableFromJson("{ filters: { Enum1: { value: [3,2], matchMode: \"in\" , operator:\"and\" }  } , first: 0, globalFilter: null, multiSortMeta: undefined, rows: 10,sortOrder: -1 }");
            var dataSet = GenerateMockTestData();
            dataSet = dataSet.PrimengTableFilter(filter, out var totalRecord);
            Assert.Equal(5, totalRecord);
        }
